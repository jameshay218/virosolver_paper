...
for(index in 1:7){
  ...
  p_dat <-  ggplot(obs_dat_use) + 
    geom_violin(aes(x=t+t_shift,group=t,y=ct),scale="width",fill="grey70",
                draw_quantiles=c(0.025,0.5,0.975)) + 
    geom_dotplot(aes(x=t, y=ct,group=t),binaxis="y",
                 binwidth=1,stackdir="center",binpositions="all",dotsize=0.25) +
    scale_y_continuous(trans="reverse") +
  ...
  p_inc <- ggplot(trajs_quants) +
    ...
    #          aes(x=t,y=detect)) +
    geom_ribbon(aes(x=t+t_shift,ymin=lower_95,ymax=upper_95),alpha=0.25,fill=AAAS_palette["blue1"]) + 
    geom_ribbon(aes(x=t+t_shift,ymin=lower_50,ymax=upper_50),alpha=0.5,fill=AAAS_palette["blue1"]) + 
...
p_dat <-  ggplot(obs_dat_use_all) + 
  geom_violin(aes(x=t+t_shift,group=t,y=ct),scale="width",fill="grey70",
              draw_quantiles=c(0.025,0.5,0.975)) + 
  geom_dotplot(aes(x=t+t_shift, y=ct,group=t),binaxis="y",
               binwidth=1,stackdir="center",binpositions="all",dotsize=0.25) +
  scale_y_continuous(trans="reverse") +
  export_theme +
...
p_comb <- ggplot(trajs_quants_all) + 
  geom_ribbon(aes(x=t+t_shift,ymin=lower_95,ymax=upper_95),alpha=0.25,fill=AAAS_palette["blue1"]) + 
  geom_ribbon(aes(x=t+t_shift,ymin=lower_50,ymax=upper_50),alpha=0.5,fill=AAAS_palette["blue1"]) + 
  geom_line(aes(x=t+t_shift,y=median),col=AAAS_palette["blue1"]) + 
  geom_line(data=tibble(t=1:200,y=(seir_outputs$inc/1000000)[1:200]),aes(x=t,y=y),col=AAAS_palette["red1"]) + 
  geom_vline(data=obs_dat_use_all,aes(xintercept=t+t_shift),linetype="dashed") +
  export_theme +
  ylab("Per capita incidence") +
  xlab("Days since start") +
  scale_x_continuous(limits=c(0,210)) +
...
a_gif <- animate(p_dat, width = 8, height = 3,units="in",res=300,duration=10,end_pause=15,start_pause=5)
b_gif <- animate(p_comb, width = 8, height = 3,units="in",res=300,duration=10,end_pause=15,start_pause=5)
...
anim_save("~/Documents/GitHub/virosolver_paper/figures/gifs/sim_recovery_late.gif",new_gif)
