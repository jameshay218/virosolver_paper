...
run_use <- 2
samp_fracs <- c(0.1,0.25,0.5,1)
#samp_fracs <- samp_fracs[1]
chain_no <- 1
...
all_gifs <- foreach(i=1:length(samp_fracs),.packages = c("extraDistr","tidyverse","patchwork",
  ...
  control_table <- control_table1 %>% filter(run_index == run_use, samp_frac==samp_frac_use,chainno==chain_no)
  ps <- NULL
  ...
  for(index in 1:12){
    ...
    chainwd <- paste0(top_chainwd,"/sim_ct_sampsize_",samp_frac,"/",run_index,"/timepoint_",timepoint)
  ...
  p_comb <- ggplot(trajs_quants_all) + 
    ...
    ggtitle(paste0("Sample size: ", samp_frac_use*2000)) +
    export_theme +
    ylab("Per capita incidence") +
    xlab("Days since start") +
    scale_x_continuous(limits=c(0,210)) +
  ...
  b_gif <- animate(p_comb, width = 5, height = 3,units="in",res=300,duration=15,end_pause=5,start_pause=5)
...
new_gif_top <- image_append(c(mgif1[1], mgif2[1]),stack=FALSE)
new_gif_bot <- image_append(c(mgif3[1], mgif4[1]),stack=FALSE)
new_gif_all <- image_append(c(new_gif_top, new_gif_bot), stack=TRUE)

for(i in 2:100){
  combined_top <- image_append(c(mgif1[i], mgif2[i]),stack=FALSE)
  combined_bot <- image_append(c(mgif3[i], mgif4[i]),stack=FALSE)
  combined_all <- image_append(c(combined_top, combined_bot), stack=TRUE)
  
  new_gif_all <- c(new_gif_all, combined_all)
}
anim_filename <- paste0("~/Documents/GitHub/virosolver_paper/figures/fullgif_samps.gif")
