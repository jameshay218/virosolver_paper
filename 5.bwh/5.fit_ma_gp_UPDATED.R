...
obs_dat_all <- read_csv("~/Documents/local_data/BWH/BWH_Panther_Cts_20200329-20211031.csv") %>% rename(panther_Ct=Ct_min,coll_date=coll_week) %>%
  mutate(platform="Panther",first_pos=1) %>%
  mutate(id=1:n()) %>%
  filter(panther_Ct < 40)
...
obs_dat1 <-  obs_dat_all %>% 
  filter(platform=="Panther" &
           first_pos %in% c(1,0)) %>%
  filter(coll_date > "2020-07-01", coll_date < "2020-11-01") %>% ## After biased symptomatic sampling time
  rename(date=coll_date) %>%
  #left_join(epi_calendar) %>%
  dplyr::select(date,  panther_Ct, id) %>%
  mutate(first_day = as.numeric(date)) %>%
...
obs_dat_all <- obs_dat_all %>% 
  ...
  dplyr::select(date,  panther_Ct, id) %>%
  arrange(date) %>%
  rename(date = date, ct=panther_Ct)
...
p_dat <- ggplot(obs_dat_all %>% filter(ct < 40)) + 
  ...
  export_theme +
  #scale_x_date(limits=as.Date(c("2020-01-01","2021-12-01")),breaks="1 month") +
  xlab("Date of sample") +
  ylab("Detectable Ct")
...
f <- create_posterior_func(parTab, obs_dat1, prior_func_use, 
                           ...
                           use_pos=TRUE,
                           t_dist=t_dist)
microbenchmark::microbenchmark(f(parTab$values))
