...
mcmcPars_ct <- c("iterations"=300000,"popt"=0.44,"opt_freq"=2000,
                 "thin"=100,"adaptive_period"=200000,"save_block"=1000)
...
parTab[parTab$names %in% c("viral_peak","obs_sd","t_switch","prob_detect"), "fixed"] <- 0

parTab[parTab$names %in% c("level_switch","wane_rate2"), "fixed"] <- 1
parTab[parTab$names == "level_switch", "values"] <- 3
parTab[parTab$names == "level_switch", c("lower_bound","lower_start")] <- 3
parTab[parTab$names == "wane_rate2", "values"] <- 100000
parTab[parTab$names == "wane_rate2", c("upper_bound","upper_start")] <- 100000
parTab[parTab$names == "t_switch","values"] <- 10
parTab[parTab$names == "prob_detect","values"] <- 0.2
...
chains_diag <- lazymcmc::load_mcmc_chains(chainwd, parTab,TRUE,1,50000,#mcmcPars_ct["adaptive_period"],
                                     multi=FALSE,chainNo=FALSE,PTchain = FALSE)

gelman_diagnostics(chains_diag$list,threshold=1.1)
chains <- lazymcmc::load_mcmc_chains(chainwd, parTab,FALSE,1,50000,#mcmcPars_ct["adaptive_period"],
...
p2 <- plot_distribution_fits(chain_comb, obs_dat1, model_func,100)
