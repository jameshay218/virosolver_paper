...
parTab[parTab$names == "obs_sd",c("lower_bound","lower_start")] <- 5
parTab[parTab$names == "obs_sd",c("upper_bound","upper_start")] <- 8
...
f <- create_posterior_func(parTab, obs_dat1 %>% filter(ct < 40), prior_func_use, 
                           inc_func_use,solve_ver="likelihood",
                           use_pos=TRUE,
                           t_dist=t_dist)
f(get_best_pars(chain_comb))
