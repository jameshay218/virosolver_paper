...
chainwd <- paste0("~/Documents/GitHub/virosolver_paper/mcmc_chains/4.real_ma_ct/",runname)
plot_wd <- paste0("~/Documents/GitHub/virosolver_paper/plots/4.real_ma_ct/",runname)
setwd(main_wd)
...
mcmcPars_ct <- c("iterations"=200000,"popt"=0.44,"opt_freq"=1000,
...
parTab[parTab$names == "prob_detect", "fixed"] <- 0
parTab[parTab$names %in% c("intercept","level_switch"), "values"] <- 38.1
...
pdf(paste0(plot_wd,"/mcmc_trace.pdf"))
...
p_dist_fits <- plot_distribution_fits(chain_comb, obs_dat1, model_func,100)
...
for(ii in seq_along(samps)){
  ...
  detect_trajs[ii,] <- sapply(ages, function(a) prop_detectable_single(a, tmp_pars,vl))
...
pdf("figures/ma_truncated_ct.pdf",height=7,width=8)
p_nyt/p_dat/p_inc  
dev.off()
